...
#load cleaned data - shld be province, year, month, extremeTP, extremeHeat, worldPopln
ccmodel <- c("MIROC6")
...
for(file in modelfiles){
  ...
  for(prov in provs){
    ...
    currdata <- pastdf %>% group_by(Month, Province) %>%
      summarise_at(vars(spi, spi_1, spi_2, spi_3, extremeHeat, extremeHeat_1, extremeHeat_2, extremeHeat_3, worldPopDens, worldPop, rh_mean), mean)
    #currdata$Province = prov; currdata$worldPop=pastdf$worldPop[1]; currdata$rh_mean=pastdf$rh_mean[1]
    
    ccdata$worldPop <- ccdata$futPop
  ...
  write.csv(ccdatapred, file=paste0("./out/withpop/", dis, "_", ccmodel, "_allCCpred.csv"))
}
...
for(file in modelfiles){
  ...
  write.csv(ccdatapred, file=paste0("./out/withpop/", dis, "_", ccmodel, "_heatCCpred.csv"))
}
...
for(file in modelfiles){
  ...
  write.csv(ccdatapred, file=paste0("./out/withpop/", dis, "_", ccmodel, "_dryCCpred.csv"))
}
...
for(file in modelfiles){
  ...
  write.csv(ccdatapred, file=paste0("./out/withpop/", dis, "_", ccmodel, "_wetCCpred.csv"))
}


###calculate annual excess risk in country
for(type in c("all", "heat", "wet", "dry")){
  for(model in c("MIROC6")){
    ccfiles <- list.files("./out/withpop/", pattern=paste0(model, "_", type, "CCpred.csv"))
    print(model)
    print(type)
    excessrisk <- list()
    for(file in ccfiles){ 
      proj <- read.csv(paste0("./out/withpop/", file))
      annualproj <- proj %>% 
        group_by(Period, SSP) %>%
    ...
    save(excessrisk, file=paste0("./out/withpop/", model, "_", type, "excessrisk.Rdata"))
  }
}
